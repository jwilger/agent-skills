...
### 0. Re-Read Role Constraints (FIRST — Before Anything Else)

Re-read the Delegation Checklist and Controller Role Boundaries sections
of the pipeline SKILL.md. You are an orchestrator, not a developer. You
never write code. This step is not optional. Crash recovery is the
highest-risk time for role drift.

### 1. Capture Surviving Agent Outputs

Before killing agents, check if any have undelivered work products. Read
`.reviews/` and any pending handoff files in the audit trail. Preserve any
...
4. **Role constraints** — Re-read the Controller Role Boundaries and
   Delegation Checklist sections of the pipeline SKILL.md (if not already
   done in Step 0).
...
### 6. Re-Spawn Agents With Full Context

When recreating TDD pair agents after compaction, the first message to
each MUST include:
- The exact phase boundaries and current TDD phase
- All process corrections from the system prompt
- Their exact SendMessage recipient names (case-sensitive)
- The specific process expectations (e.g., "You are in RED phase. You may
  ONLY edit test files.")

Do not assume fresh agents inherit behavioral context from predecessors.
