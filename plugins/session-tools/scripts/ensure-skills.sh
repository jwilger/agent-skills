...
REQUIRED_SKILLS=(session-reflection memory-protocol error-recovery)
MISSING=()
...
if [[ ${#MISSING[@]} -eq 0 ]]; then
  echo '{"systemMessage": "session-tools plugin: all required skills present."}'
  exit 0
fi
...
echo "{\"systemMessage\": \"session-tools plugin auto-installed missing skills: ${MISSING[*]}. Verify they loaded correctly.\"}"
